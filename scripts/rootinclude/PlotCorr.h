...
public:
	PlotCorr();
	PlotCorr(const string& basedir, const string& model);
	void add_fit(std::string subdir, std::string target, std::string type, int udate, int idate1, int idate2);
...
private:
	...
	void get_series(std::vector<int>& vd, std::vector<double>& v, int vindx, std::string sprd, double& grmax, double& grmin);
};
...
PlotCorr::PlotCorr()
{
	...
	PlotCorr::PlotCorr(const string& basedir, const string& model)
:basedir_(basedir),
	model_(model),
	nfits_(0),
	nvar_(0)
...
void PlotCorr::add_fit(std::string subdir, std::string target, std::string type, int udate, int idate1, int idate2)
{
	++nfits_;
...
void PlotCorr::add_var(std::string var)
{
	++nvar_;
	vars_.push_back(var);
}

int PlotCorr::get_nmonths(int idate0, int idate1)
{
...
void PlotCorr::get_ma(std::vector<double>& vMA, std::vector<double>& v, int nma)
{
	int N = v.size();
	if( N > nma )
	{
...
int PlotCorr::get_max_ntrees(const string& path)
{
	int max_ntrees = 0;
	...
	for( int iv = 0; iv < nvar_; ++iv )
	{
		...
		for( int it = 0; it < nfits_; ++it )
		{
			...
			for( int is = 0; is < NS; ++is )
			{
				...
				char title[100];
				sprintf(title, "%s %s %s %s, %d day MA", var.c_str(), model_.c_str(), types_[it].c_str(), sprds_[is].c_str(), nma);
...
void PlotCorr::read_lines(char* path)
{
	lines_.clear();
	vindx_.clear();
	int max_ntrees = get_max_ntrees(path);
	ifstream ifs(path);
	if( ifs.is_open() )
	{
		string line;
		while( getline(ifs, line) )
		{
			...
			{
				if( max_ntrees == atoi(sl[11].c_str()) )
...
void PlotCorr::get_series(vector<int>& vd, vector<double>& v, int vindx, string sprd, double& grmax, double& grmin)
{
	double last_val = 0.;
