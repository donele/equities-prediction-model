...
if [[ "x$nmax" == "x" ]]; then
	sigDump -n 10000000 -d $idate -v $varList | awk '((NR-1)%'$sampleFreq'==0){print $0}' > temp_sig.txt
	head -n 10000001 ../../../$model/fit/$targetName/stat_${udate}/preds/pred${idate}.txt | awk '((NR-1)%'$sampleFreq'==0){print $3}' > temp_pred.txt
else
	sampleFreq=1
	echo $nmax
	nmaxp1=$(( $nmax + 1 ))
	sigDump -n $nmax -d $idate -v $varList | awk '((NR-1)%'$sampleFreq'==0){print $0}' | head -n $nmax > temp_sig.txt
	head -n $nmaxp1 ../../../$model/fit/$targetName/stat_${udate}/preds/pred${idate}.txt | awk '((NR-1)%'$sampleFreq'==0){print $3}' | head -n $nmax > temp_pred.txt
fi

nlinesSig=`wc temp_sig.txt | awk '{print $1}'`
nlinesPred=`wc temp_pred.txt | awk '{print $1}'`
if [[ $nlinePred > 0 && $nlinesSig != $nlinesPred ]]; then
