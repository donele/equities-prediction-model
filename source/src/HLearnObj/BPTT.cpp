...
namespace hnn {
	...
	BPTT::BPTT(vector<int>& N, double learn, double wgtDecay, double maxWgt, int verbose, double biasOut, double costMult,
	...
	vector<double> BPTT::train(const vector<float>& input, const QuoteInfo& quote, const double target)
	{
	...
	vector<double> BPTT::trading_signal(const vector<float>& input, const QuoteInfo& quote, const double lastPos)
	{
		P_p_ = lastPos;
		vector<double> output = forward(input, quote);
	...
	std::ostream& BPTT::write(ostream& str) const
	{
	...
	std::istream& BPTT::read(istream& str)
	{
		str >> L_;
	...
	vector<double> BPTT::forward(const vector<float>& input, const QuoteInfo& quote)
	{
		quote_ = quote;
