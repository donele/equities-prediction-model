...
namespace hnn {
	BPTTpred2inD::BPTTpred2inD()
	{
		get_iPred();
	}

	BPTTpred2inD::BPTTpred2inD(const BPTTpred2inD& rhs)
	:NNBase(rhs)
	{
		sigP_ = rhs.sigP_;
		poslim_ = rhs.poslim_;
	...
	BPTTpred2inD::BPTTpred2inD(vector<int>& N, double learn, double wgtDecay, double maxWgt, int verbose, double biasOut, double costMult,
	...
	void BPTTpred2inD::init_vectors()
	{
		// pFpw_.
	...
	void BPTTpred2inD::beginDay()
	{
		get_iPred();
		return;
	}

	void BPTTpred2inD::get_iPred()
	{
	...
	double BPTTpred2inD::costMult()
	{
		return costMult_;
	}

	void BPTTpred2inD::start_ticker(double position)
	{
		last_mid_ = 0;
		P_p_ = position;
	...
	vector<double> BPTTpred2inD::train(const vector<float>& input, const QuoteInfo& quote, const double target)
	{
		...
		output.push_back(F_ + P_p_);

		return output;
	}

	vector<double> BPTTpred2inD::trading_signal(const vector<float>& input, const QuoteInfo& quote, const double lastPos)
	{
		P_p_ = lastPos;
		vector<double> output = forward(input, quote);
	...
	std::ostream& BPTTpred2inD::write(ostream& str) const
	{
	...
	std::istream& BPTTpred2inD::read(istream& str)
	{
		str >> L_;
	...
	vector<double> BPTTpred2inD::forward(const vector<float>& input, const QuoteInfo& quote)
	{
		quote_ = quote;
	...
	void BPTTpred2inD::backprop(double error)
	{
