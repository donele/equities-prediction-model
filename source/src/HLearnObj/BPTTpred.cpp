...
namespace hnn {
	...
	BPTTpred::BPTTpred(const BPTTpred& rhs)
	:NNBase(rhs)
	{
		sigP_ = rhs.sigP_;
		poslim_ = rhs.poslim_;
	...
	BPTTpred::BPTTpred(vector<int>& N, double learn, double wgtDecay, double maxWgt, int verbose, double biasOut, double costMult,
	...
	void BPTTpred::beginDay()
	{
		get_iPred();
		return;
	}

	void BPTTpred::get_iPred()
	{
	...
	double BPTTpred::costMult()
	{
		return costMult_;
	}

	void BPTTpred::start_ticker(double position)
	{
		last_mid_ = 0;
		P_p_ = position;
	...
	vector<double> BPTTpred::train(const vector<float>& input, const QuoteInfo& quote, const double target)
	{
		...
		// Position
		double pred = (pred1m + pred40m) / 10000;
		double cost = (quote.ask - quote.bid) / (quote.ask + quote.bid) * costMult_;
		double pos = P_p_;

		if( pred - cost > 0 )
			pos = P_p_ + quote.askSize * lotSize_;
		else if( pred + cost < 0 )
			pos =  P_p_ - quote.bidSize * lotSize_;
	...
	vector<double> BPTTpred::trading_signal(const vector<float>& input, const QuoteInfo& quote, const double lastPos)
	{
		P_p_ = lastPos;
		vector<double> output = forward(input, quote);
	...
	std::ostream& BPTTpred::write(ostream& str) const
	{
	...
	std::istream& BPTTpred::read(istream& str)
	{
		str >> L_;
	...
	vector<double> BPTTpred::forward(const vector<float>& input, const QuoteInfo& quote)
	{
		quote_ = quote;
	...
	void BPTTpred::backprop(double error)
	{
