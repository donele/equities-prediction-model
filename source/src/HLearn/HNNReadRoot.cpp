...
HNNReadRoot::HNNReadRoot(const string& moduleName, const multimap<string, string>& conf)
...
threadSampleRead_(false)
{
	...
	if( conf.count("inputName") )
	{
		pair<mmit, mmit> names = conf.equal_range("inputName");
		for( mmit mi = names.first; mi != names.second; ++mi )
		{
	...
	{
		cerr << "Error HNNReadRoot::threadSampleRead_ should be true when HEnv::multiThreadTicker is true." << endl;
		exit(5);
	}
...
HNNReadRoot::~HNNReadRoot()
{}

void HNNReadRoot::beginJob()
{
	cout << moduleName_ << "::beginJob()" << endl;
	HEvent::Instance()->add<vector<string> >("", "inputNames", inputNames_);

	// Create InputInfo with number of inputs only.
...
void HNNReadRoot::beginDay()
{
	...
	if( threadSampleRead_ )
		thread_ = boost::shared_ptr<boost::thread>(new boost::thread(boost::bind(&HNNReadRoot::startThreadSampleRead, this)));
	...
	if( verbose_ >= 1 )
	{
		...
		{
			cout << "\n";
			cout << "HNNReadRoot::beginDay " << idate << ". " << "ticker(s): ";
			if( ts > 10 )
			{
...
void HNNReadRoot::beginTicker(const string& ticker)
{
	if( threadSampleRead_ )
	{
		while(1)
...
void HNNReadRoot::endTicker(const string& ticker)
{
...
void HNNReadRoot::read_sample(string ticker)
{
	...
	{
		cerr << "HNNReadRoot::read_sample read '" << sh.ticker << "' while expecting '" << ticker << "'" << endl;
		exit(5);
	}
...
void HNNReadRoot::normalize_sample(vector<hnn::Sample>& sample)
{
...
void HNNReadRoot::startThreadSampleRead()
{
