...
SignalCalculatorMicro::SignalCalculatorMicro()
	:debug_(false),
	pStatusChange_(nullptr)
{
}

SignalCalculatorMicro::SignalCalculatorMicro(int idate, const string& uid, const string& ticker, SigC& sig,
...
: TCS_book_micro(ticker,UsecsTime(openMsecs),UsecsTime(closeMsecs),auctionMkts),
	...
	max_trade_qty_rat_(0.),
	max_trade_price_rat_(0.),
	quoteSampleStep_(0),
	pStatusChange_(nullptr),
	idate_(idate),
	tradeSampleStep_(0),
...
void SignalCalculatorMicro::setDebug()
{
	debug_ = true;
}

void SignalCalculatorMicro::setDiscardOutliers(bool tf)
{
	discardOutliers_ = tf;
}

void SignalCalculatorMicro::setStatusChange(StatusChangeUS* p)
{
	pStatusChange_ = p;
}

void SignalCalculatorMicro::setOpenDelay(int delay)
{
	openDelay_ = delay;
}

void SignalCalculatorMicro::setQuoteSampleStep(int step)
{
	quoteSampleStep_ = step;
	nbboCounter_ = SampleCounter(quoteSampleStep_);
}

void SignalCalculatorMicro::setTradeSampleStep(int step)
{
	tradeSampleStep_ = step;
	tradeCounter_ = SampleCounter(tradeSampleStep_);
}

void SignalCalculatorMicro::setBooktradeSampleStep(int step)
{
	booktradeSampleStep_ = step;
	booktradeCounter_ = SampleCounter(booktradeSampleStep_);
}

const vector<QuoteInfo>& SignalCalculatorMicro::getQuotes() const
{
	return quotes_;
}

const TradeInfo& SignalCalculatorMicro::getFirstTrade() const
{
	return firstTrade_;
}

const QuoteInfo& SignalCalculatorMicro::getFirstQuote() const
{
	return firstQuote_;
}

void SignalCalculatorMicro::endTicker()
{
	qSample_.endTicker();
}

void SignalCalculatorMicro::addQuote(const QuoteInfo& quote)
{
	quotes_.push_back(quote);
	qSample_.addQuote(quote);
}

void SignalCalculatorMicro::addTrade(const TradeInfo& trade)
{
...
void SignalCalculatorMicro::addBookTrade(const TradeInfo& trade)
{
...
void SignalCalculatorMicro::setTmSampleMsecs(const vector<int>& sampleMsecs, const int scale)
{
...
void SignalCalculatorMicro::NbboCB(UsecsTime ti,const QuoteDataMicro& qdm)
{
...
void SignalCalculatorMicro::TradeCB(UsecsTime ti,const TradeDataMicro& trade)
{
...
void SignalCalculatorMicro::BookTradeCB(UsecsTime ti,const TradeDataMicro& trade)
{
...
void SignalCalculatorMicro::RegularCB(UsecsTime ti)//fundamentally the same
{
	int msecs = ti.Msecs();
	if( !pSessions_->isAfterOpenBeforeClose(msecs) )
		return;
...
void SignalCalculatorMicro::setPersistence(int iSig)
{
...
bool SignalCalculatorMicro::NewSig(int msecs, int sampleType)
{
...
void SignalCalculatorMicro::calculate_trade_signals(hff::SigC& sig, hff::StateInfo& sI, int msecs)
{
	...
	QuoteInfo priorQuote = nbboBeforeLastBookTrade_;
...
double SignalCalculatorMicro::get_mret_upto1s(int msecs,
...
void SignalCalculatorMicro::calculate_quote_signals(hff::SigC& sig, hff::StateInfo& sI, int msecs)
{
...
void SignalCalculatorMicro::calculate_fillimb_signals(hff::StateInfo& sI, int msecs, int sampleType)
{
	const hff::LinearModel& linMod = MEnv::Instance()->linearModel;
...
float SignalCalculatorMicro::get_fillImb(const QuoteInfo& quote, const TradeInfo& trade)
{
	float ret = 0.;
...
float SignalCalculatorMicro::get_fillImb()
{
	QuoteInfo quote = Bbo( (unsigned char)lastBookTrade_.flags );
	string market = string(1, lastBookTrade_.flags);
...
void SignalCalculatorMicro::calculate_targets(hff::SigC& sig, const vector<QuoteInfo>& quotes, bool smoothTarget)
{
	const hff::LinearModel& linMod = MEnv::Instance()->linearModel;
	const hff::NonLinearModel& nonLinMod = MEnv::Instance()->nonLinearModel;
...
void SignalCalculatorMicro::calculate_tob_signals(hff::StateInfo& sI)
{
...
void SignalCalculatorMicro::calculate_book_signals(hff::SigC& sig, hff::StateInfo& sI)
{
	if( sI.tobOK == 1 )
	{
...
void SignalCalculatorMicro::calculate_bookcnt_signals(hff::SigC& sig, hff::StateInfo& sI)
{
	if( sI.tobOK == 1 )
	{
		float tickSize = .01f;
...
void SignalCalculatorMicro::calculate_mi_signals(hff::SigC& sig, hff::StateInfo& sI)
{
...
void SignalCalculatorMicro::calculate_signals(hff::SigC& sig, hff::StateInfo& sI, int msecs)
{
	const hff::LinearModel& linMod = MEnv::Instance()->linearModel;
	float firstMidQt = get_mid(firstQuote_);
...
void SignalCalculatorMicro::get_top_comp_book(QuoteInfo& compBkTop)
{
...
TaskPersistence::TaskPersistence(SignalCalculatorMicro* sigcal, int iSig)
	:iSig_(iSig),
	sigcal_(sigcal)
{
}
TaskPersistence* TaskPersistence::MakeCopy(void)
{
	return new TaskPersistence(sigcal_, iSig_);
}

void TaskPersistence::Execute(UsecsTime ti)
{
