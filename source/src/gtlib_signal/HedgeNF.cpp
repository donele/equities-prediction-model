...
HedgeNF::HedgeNF(int idate)
	:idate_(idate)
{
}

const TargetHedger* HedgeNF::getTargetHedger(const string& ticker)
{
...
void HedgeNF::calculateHedge(int n1sec)
{
	...
	for( int k = 0; k < n1sec - 1; ++k )
	{
		...
		for( int i = 0; i < hedgeN; ++i )
		{
			...
			if( it == mTargetHedger_.end() )
			{
				mTargetHedger_[ticker] = TargetHedgerNF(n1sec);
...
void HedgeNF::getNFSignals(NorthfieldHedge& nfh, int n1sec)
{
	...
	for( int i = 0; i < totN; ++i )
	{
		...
		if( nfh.GetParams(sigh.ticker, &nfParams) && nfParams.sector >= 0 )
		{
			...
			mTargetHedger_[sigh.ticker] = TargetHedgerNF(n1sec, northBP, northTRD, northRST);
