...
HOrderSummRead::HOrderSummRead(const string& moduleName, const multimap<string, string>& conf)
...
routing_("")
{
	...
	if( conf.count("plotCand") )
		plotCand_ = conf.find("plotCand")->second == "true";
...
HOrderSummRead::~HOrderSummRead()
{}

void HOrderSummRead::beginJob()
{
...
void HOrderSummRead::beginMarket()
{
	...
	if( plotCand_ )
	{
		char name[100];
		char title[200];
		sprintf(name, "h_dmsec");
		sprintf(title, "dmsec, %s", market.c_str());
		h_dmsec_ = TH1F(name, title, 1000, -500, 500 );

		sprintf(name, "h_ncand10");
		sprintf(title, "ncand10, %s", market.c_str());
		h_ncand10_ = TH1F(name, title, 20, 0, 20 );

		sprintf(name, "h_ncand9");
		sprintf(title, "ncand9, %s", market.c_str());
		h_ncand9_ = TH1F(name, title, 20, 0, 20 );

		sprintf(name, "h_ncand8");
		sprintf(title, "ncand8, %s", market.c_str());
		h_ncand8_ = TH1F(name, title, 20, 0, 20 );

		sprintf(name, "h_ncand7");
		sprintf(title, "ncand7, %s", market.c_str());
		h_ncand7_ = TH1F(name, title, 20, 0, 20 );

		sprintf(name, "h_ncand6");
		sprintf(title, "ncand6, %s", market.c_str());
		h_ncand6_ = TH1F(name, title, 20, 0, 20 );

		sprintf(name, "h_ncand5");
		sprintf(title, "ncand5, %s", market.c_str());
		h_ncand5_ = TH1F(name, title, 20, 0, 20 );
	}
...
void HOrderSummRead::beginMarketDay(string market, int idate)
{
...
void HOrderSummRead::beginTicker(const string& ticker)
{
...
void HOrderSummRead::endTicker(const string& ticker)
{
...
void HOrderSummRead::endDay()
{
}

void HOrderSummRead::endMarket()
{
	string market = HEnv::Instance()->market();
...
void HOrderSummRead::endJob()
{
}

void HOrderSummRead::set_ticker_list(int idate, string market)
{
...
void HOrderSummRead::get_retON(string market, int idate)
{
...
void HOrderSummRead::read_orders(vector<vector<string> >& vvo, int idate, string market, string ticker)
{
	vvo.clear();
	string sExch = mto::code(market);
...
string HOrderSummRead::get_selMarket(string market)
{
	string sExch = mto::code(market);
...
void HOrderSummRead::select_orders(vector<vector<string> >& vvo, vector<MercatorOrder>& vMOrderBuy, vector<MercatorOrder>& vMOrderSell,
...
void HOrderSummRead::set_latency_offset(int idate, string market)
{
	latency_offset_ = 0;
...
void HOrderSummRead::read_quote_info( vector<MercatorOrder>& vMOrder, string market, string ticker, string bs,
...
void HOrderSummRead::calc_returns(TickSeries<QuoteInfo>& tsQ, int orderMsecs, vector<MercatorOrder>::iterator& itb, const int mult, int closeMsecs, double reton)
{
	// Returns.
...
void HOrderSummRead::find_match(QuoteInfo& theQuote, TickSeries<QuoteInfo>& tsQ, int orderMsecs, vector<MercatorOrder>::iterator& itb, string bs)
{
	...
	if( !mTightMM.empty() )
	{
		...
		if( plotCand_ )
		{
			h_dmsec_.Fill((orderMsecs - q2oMin_ + latency_offset_) - theQuote.msecs);
			h_ncand10_.Fill(mTightMM.size());
		}
	}
	else if( !mLooseMM.empty() )
	{
		...
		{
			h_dmsec_.Fill((orderMsecs - q2oMin_ + latency_offset_) - theQuote.msecs);
			h_ncand9_.Fill(mLooseMM.size() + mLooseMP.size());
		}
	}
	...
	else if( !mLooseMP.empty() )
	{
		...
		{
			h_dmsec_.Fill((orderMsecs - q2oMin_ + latency_offset_) - theQuote.msecs);
			h_ncand7_.Fill(mLooseMM.size() + mLooseMP.size());
		}
	}
	...
	else if( !mLooseM.empty() )
	{
		...
		{
			h_dmsec_.Fill((orderMsecs - q2oMin_ + latency_offset_) - theQuote.msecs);
			h_ncand5_.Fill(mLooseM.size());
		}
	}
}

bool HOrderSummRead::exchangeMatch(int tickEx, int dbEx)
{
	return true;
...
bool HOrderSummRead::is_valid( QuoteInfo& quote )
{
...
int HOrderSummRead::get_price_change( double last, double now )
{
	int ret = 0;
...
bool HOrderSummRead::valid_price( double price )
{
	bool valid = price > ltmb_ && price < 10000000;
	return valid;
}

void HOrderSummRead::get_o2x( vector<MercatorOrder>& vMOrder, int m1, int m2, double price, int priceDeteriorated )
{
