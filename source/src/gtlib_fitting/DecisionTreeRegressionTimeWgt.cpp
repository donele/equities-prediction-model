...
DecisionTreeRegressionTimeWgt::DecisionTreeRegressionTimeWgt(DecisionTreeParam decisionTreeParam, FitData* pFitData)
...
DecisionTreeRegressionTimeWgt::DecisionTreeRegressionTimeWgt(DecisionTreeParam decisionTreeParam, FitData* pFitData,
		shared_ptr<vector<vector<int>>> pSortedIndex, float decayFactor)
...
DecisionTreeRegressionTimeWgt::DecisionTreeRegressionTimeWgt(vector<string>::const_iterator& itFrom, vector<string>::const_iterator itTo)
{
	pRoot_ = make_node(itFrom, itTo);
}

shared_ptr<DTNode> DecisionTreeRegressionTimeWgt::make_node(vector<string>::const_iterator& itFrom, vector<string>::const_iterator& itTo)
{
	if( itFrom == itTo )
	{
...
void DecisionTreeRegressionTimeWgt::fit()
{
	fit_initialize();
...
void DecisionTreeRegressionTimeWgt::fit_initialize()
{
	if( pRoot_ != nullptr )
	{
...
void DecisionTreeRegressionTimeWgt::fit_node_stat()
{
	...
	for( int iSample = 0; iSample < nSamplePoints_; ++iSample )
	{
		...
		if( iNodeBegin_ <= iNode && iNode < iNodeEnd_ )
		{
			...
			vAccTarget_[iNode].add(target, getWgt(iSample));
		}
	}
...
void DecisionTreeRegressionTimeWgt::fit_node_cut()
{
	// single thread.
	...
	else // multi thread. (test result 35 -> 10)
	{
		vector<thread> vt;
		for( int iInput = 0; iInput < nInputFields_; ++iInput )
			vt.push_back(thread(&DecisionTreeRegressionTimeWgt::fit_node_cut_input, this, iInput));
		for( auto& t : vt )
			t.join();
	}
	...
	else // multi thread. (test result 10 -> 10)
	{
		vector<thread> vt;
		for( int iNode = iNodeBegin_; iNode < iNodeEnd_; ++iNode )
			vt.push_back(thread(&DecisionTreeRegressionTimeWgt::fit_node_cut_recombine, this, iNode));
		for( auto& t : vt )
			t.join();
	}
}

void DecisionTreeRegressionTimeWgt::fit_node_cut_input(int iInput)
{
	...
	for( int iSample = 0; iSample < nSamplePoints_; ++iSample )
	{
		...
		stat.tmpXold = tmpX;
		stat.acc.add(pFitData_->target(cIndx), getWgt(cIndx));
	}
}

void DecisionTreeRegressionTimeWgt::fit_node_cut_recombine(int iNode)
{
...
double DecisionTreeRegressionTimeWgt::TnodeCutFn(const WAcc& acc, const WAcc& accTot, int weirdFactor)
{
	/*
...
double DecisionTreeRegressionTimeWgt::getWgt(int i)
{
	double wgt = exp((i - nSamplePoints_)/(decayFactor_ * nSamplePoints_));
	return wgt;
}

bool DecisionTreeRegressionTimeWgt::fit_node_create()
{
...
double DecisionTreeRegressionTimeWgt::pred(int iSample)
{
	vector<float> input = pFitData_->getSample(iSample);
	return pred(input);
}

double DecisionTreeRegressionTimeWgt::pred(const vector<float>& input)
{
	return traverse_mu(pRoot_, input);
}

double DecisionTreeRegressionTimeWgt::traverse_mu(shared_ptr<DTNode> pnode, const vector<float>& input)
{
...
double DecisionTreeRegressionTimeWgt::get_deviance()
{
...
void DecisionTreeRegressionTimeWgt::traverse_deviance(shared_ptr<DTNode> pnode, double& sumDeviance, int& npts)
{
...
void DecisionTreeRegressionTimeWgt::deviance_improvement(vector<double>& stats)
{
	traverse_improvement(pRoot_, stats);
}

void DecisionTreeRegressionTimeWgt::traverse_improvement(shared_ptr<DTNode> pnode, vector<double>& stats)
{
...
void DecisionTreeRegressionTimeWgt::writeModel(const string& path)
{
}
