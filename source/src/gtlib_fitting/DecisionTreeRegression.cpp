...
DecisionTreeRegression::DecisionTreeRegression(DecisionTreeParam decisionTreeParam, FitData* pFitData)
...
DecisionTreeRegression::DecisionTreeRegression(vector<string>::const_iterator& itFrom, vector<string>::const_iterator itTo)
{
	pRoot_ = make_node(itFrom, itTo);
}

//shared_ptr<DTNode> DecisionTreeRegression::make_node(vector<string>::const_iterator& itFrom, vector<string>::const_iterator& itTo)
//{
//	if( itFrom == itTo )
//	{
...
void DecisionTreeRegression::fit()
{
	fit_initialize();
...
void DecisionTreeRegression::fit_initialize()
{
	if( pRoot_ != nullptr )
	{
...
void DecisionTreeRegression::fit_node_stat()
{
	...
	for( int iNode = iNodeBegin_; iNode < iNodeEnd_; ++iNode )
	{
		...
		const Acc& acc = vAccTarget_[iNode];
...
void DecisionTreeRegression::fit_node_cut()
{
	// single thread.
	...
	else // multi thread. (test result 35 -> 10)
	{
		vector<thread> vt;
		for( int iInput = 0; iInput < nInputFields_; ++iInput )
			vt.push_back(thread(&DecisionTreeRegression::fit_node_cut_input, this, iInput));
		for( auto& t : vt )
			t.join();
	}
	...
	else // multi thread. (test result 10 -> 10)
	{
		vector<thread> vt;
		for( int iNode = iNodeBegin_; iNode < iNodeEnd_; ++iNode )
			vt.push_back(thread(&DecisionTreeRegression::fit_node_cut_recombine, this, iNode));
		for( auto& t : vt )
			t.join();
	}
}

void DecisionTreeRegression::fit_node_cut_input(int iInput)
{
...
void DecisionTreeRegression::fit_node_cut_recombine(int iNode)
{
	...
	for( int iInput = 0; iInput < nInputFields_; ++iInput )
	{
		const DTStat& stat = vvStat_[iInput][iNode];
	...
	if( cutIndex >= 0 )
	{
		const DTStat& stat = vvStat_[cutIndex][iNode];
...
double DecisionTreeRegression::TnodeCutFn(const Acc& acc, const Acc& accTot, int weirdFactor)
{
	/*
	...
	double right_var = (accTot.sum2 - acc.sum2) - ((accTot.sum - acc.sum) * (accTot.sum - acc.sum) / (accTot.n - acc.n));

	return(-left_var - right_var);
}

bool DecisionTreeRegression::fit_node_create()
{
...
double DecisionTreeRegression::pred(int iSample)
{
	vector<float> input = pFitData_->getSample(iSample);
	return pred(input);
}

double DecisionTreeRegression::pred(const vector<float>& input)
{
	return traverse_mu(pRoot_, input);
}

double DecisionTreeRegression::traverse_mu(shared_ptr<DTNode> pnode, const vector<float>& input)
{
...
double DecisionTreeRegression::get_deviance()
{
...
void DecisionTreeRegression::traverse_deviance(shared_ptr<DTNode> pnode, double& sumDeviance, int& npts)
{
...
void DecisionTreeRegression::deviance_improvement(vector<double>& stats)
{
	traverse_improvement(pRoot_, stats);
}

void DecisionTreeRegression::traverse_improvement(shared_ptr<DTNode> pnode, vector<double>& stats)
{
...
void DecisionTreeRegression::writeModel(const string& path)
{
}
