...
MTickReadSimple::MTickReadSimple(const string& moduleName, const multimap<string, string>& conf)
	...
	remove_break_(false)
{
	...
	if( conf.count("offset") )
		offset_ = atoi( conf.find("offset")->second.c_str() );
...
MTickReadSimple::~MTickReadSimple()
{}

void MTickReadSimple::beginJob()
{
...
void MTickReadSimple::beginMarketDay()
{
	...
	if(offset_ > 0)
	{
		...
		for(int i = 0; i < offset_; ++i)
			readDate_ = nextOpen(market, readDate_);
	}
	else if(offset_ < 0)
	{
		ext_ = "-" + itos(-offset_);
		for(int i = 0; i < -offset_; ++i)
			readDate_ = prevClose(market, readDate_);
	}
	longTicker_ = mto::longTicker(market);
	sessions_ = Sessions(market, readDate_);

	set_directories(market);
	...
	{
		vector<string> univ_tickers = get_univ_tickers(market, readDate_);
	...
	if( verbose_ > 3 )
	{
		...
		int ts = tickers.size();
		cout << "\n";
		cout << "market: " << market << ", date: " << readDate_ << ". " << "ticker(s): ";
		if( ts > 10 )
		{
	...
	if( verbose_ > 0 )
		cout << "\MTickReadSimple::beginMarketDay() " << readDate_ << " " << market << ".\n";
	if( verbose_ > 1 )
		PrintMemoryInfoSimple();
	return;
}

void MTickReadSimple::beginTicker(const string& ticker)
{
	if(!preload_)
		readTickData(ticker);
}

void MTickReadSimple::readTickData(const string& ticker)
{
	string market = MEnv::Instance()->market;
	int nq = 0;
	int nn = 0;
	...
	if( siTrade_.do_read )
		read_data<TradeInfo>(siTrade_.dirs, "trades"+ext_, nt, ticker, readDate_);
	if( siQuote_.do_read )
		read_data<QuoteInfo>(siQuote_.dirs, "quotes"+ext_, nq, ticker, readDate_);
	if( siNbbo_.do_read )
		read_data<QuoteInfo>(siNbbo_.dirs, "nbbo"+ext_, nn, ticker, readDate_);
	if( siSip_.do_read )
		read_data<QuoteInfo>(siSip_.dirs, "sip"+ext_, ns, ticker, readDate_);
	if( siOrder_.do_read )
		read_data<OrderData>(siOrder_.dirs, "orders"+ext_, no, ticker, readDate_);
	if( siNews_.do_read )
		read_data<QuoteInfo>(siNews_.dirs, "news"+ext_, nw, ticker, readDate_);
...
void MTickReadSimple::endTicker(const string& ticker)
{
	if( siQuote_.do_read )
		MEvent::Instance()->remove<vector<QuoteInfo> >(ticker, "quotes"+ext_);
	if( siNbbo_.do_read )
		MEvent::Instance()->remove<vector<QuoteInfo> >(ticker, "nbbo"+ext_);
	if( siSip_.do_read )
		MEvent::Instance()->remove<vector<QuoteInfo> >(ticker, "sip"+ext_);
	if( siTrade_.do_read )
		MEvent::Instance()->remove<vector<TradeInfo> >(ticker, "trades"+ext_);
	if( siOrder_.do_read )
		MEvent::Instance()->remove<vector<OrderData> >(ticker, "orders"+ext_);
	if( siNews_.do_read )
		MEvent::Instance()->remove<vector<QuoteInfo> >(ticker, "news"+ext_);
}

void MTickReadSimple::endMarketDay()
{
	string market = MEnv::Instance()->market;
	if( verbose_ > 2 )
	{
		cout << "MTickReadSimple::endMarketDay() " << readDate_ << " " << market << ".\n";
		PrintMemoryInfoSimple();
	}
}

void MTickReadSimple::endJob()
{
}

void MTickReadSimple::set_directories(const std::string& market)
{
	if( siQuote_.do_read )
	{
		if( siQuote_.dir_given )
			...
			siQuote_.dirs = tSources_.stocksdirectory(market, readDate_);
	}
	if( siNbbo_.do_read )
	{
		if( siNbbo_.dir_given )
			...
			siNbbo_.dirs = tSources_.nbbodirectory(market, readDate_);
	}
	if( siSip_.do_read )
	{
		if( siSip_.dir_given )
			...
			siSip_.dirs = tSources_.sipdirectory(market, readDate_);
	}
	if( siTrade_.do_read )
	{
		if( nbbo_trade_ )
		...
		else
		{
			...
			else
			{
				if( siQuote_.dir_given ) // If quote dir is given, use it for the trade, too.
					...
					siTrade_.dirs = tSources_.nbbodirectory(market, readDate_);
			}
		}
	}
	if( siOrder_.do_read )
	{
		if( siOrder_.dir_given )
			...
			siOrder_.dirs = tSources_.bookdirectory(market, readDate_);
	}
	if( siNews_.do_read )
	{
		...
		//else
		//	siNews_.dirs = tSources_.newsdirectory(market, readDate_);
	}
}
