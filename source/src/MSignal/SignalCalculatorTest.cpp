...
SignalCalculatorTest::SignalCalculatorTest()
	:debug_fillImbBook_(false)
{
}

SignalCalculatorTest::SignalCalculatorTest(const string& uid, const string& ticker, SigC& sig,
...
const vector<QuoteInfo>& SignalCalculatorTest::getQuotes() const
{
	return quotes_;
}

const TradeInfo& SignalCalculatorTest::getFirstTrade() const
{
	return firstTrade_;
}

const QuoteInfo& SignalCalculatorTest::getFirstQuote() const
{
	return firstQuote_;
}

void SignalCalculatorTest::endTicker()
{
	qSample_.endTicker();
}

void SignalCalculatorTest::debug_fillImbBook(TickSources& ts, const vector<string>& markets, int idate, const string& ticker)
{
...
void SignalCalculatorTest::debug_booktrade_prob()
{
	debug_booktrade_prob_ = true;
}

void SignalCalculatorTest::addQuote(const QuoteInfo& quote)
{
	quotes_.push_back(quote);
	qSample_.addQuote(quote);
}

void SignalCalculatorTest::addTrade(const TradeInfo& trade)
{
	trades_.push_back(trade);

	int msecs_15min = 900000 + openMsecs_;
	//if( firstQuote_.msecs > 0 && trade.msecs >= firstQuote_.msecs && trade.msecs < msecs_15min )
...
void SignalCalculatorTest::NbboCB(int msecs, TickProvider<TradeInfo,QuoteInfo,OrderData>* provider)
{
	...
	if( adjPrevClose_ > 0. && msecs < sec_on_ * 1000 + openMsecs_ )
	{
		if( valid_quote(lastNbbo_) )
			ret_on_ = basis_pts_ * (get_mid(lastNbbo_) / adjPrevClose_ - 1.);
		else
			ret_on_ = 0.;
	}
...
void SignalCalculatorTest::TradeCB(int msecs, TickProvider<TradeInfo,QuoteInfo,OrderData>* provider)
{
	if( !pSessions_->isAfterOpenBeforeClose(msecs) )
		return;
...
void SignalCalculatorTest::BookTradeCB(int msecs, TickProvider<TradeInfo,QuoteInfo,OrderData>* provider)
{
	if( !pSessions_->isAfterOpenBeforeClose(msecs) )
		return;
...
void SignalCalculatorTest::RegularCB(int msecs, TickProvider<TradeInfo,QuoteInfo,OrderData>* provider)
{
	if( !pSessions_->isAfterOpenBeforeClose(msecs) )
		return;
...
void SignalCalculatorTest::TimeCB(int msecs, TickProvider<TradeInfo,QuoteInfo,OrderData>* provider, void* ref)
{
...
bool SignalCalculatorTest::NewSig(int msecs, TickProvider<TradeInfo,QuoteInfo,OrderData>* provider, int sampleType)
{
...
void SignalCalculatorTest::calculate_trade_signals(hff::SigC& sig, hff::StateInfo& sI, int msecs)
{
	...
	// vm and vwap.
	sI.voluMom600 = get_voluMom(msecs, 600, trades_);
	sI.voluMom3600 = get_voluMom(msecs, 3600, trades_);
	sI.intraVoluMom = get_voluMom(msecs, 0, trades_);

	if(sig.avgDlyVolume > 0) // Normalize volume momentum signals.
	{
		sI.voluMom600  = sI.voluMom600 * f600_ / sig.avgDlyVolume;
		sI.voluMom3600 = sI.voluMom3600 * f3600_ / sig.avgDlyVolume;
		if( sec > 0 )
		{
...
double SignalCalculatorTest::get_mret_upto1s(int msecs,
...
void SignalCalculatorTest::calculate_quote_signals(hff::SigC& sig, hff::StateInfo& sI,
...
void SignalCalculatorTest::calculate_fillimb_signals(hff::StateInfo& sI, int msecs,
...
void SignalCalculatorTest::calculate_exploratory_signals(hff::StateInfo& sI, int msecs,
		TickProvider<TradeInfo, QuoteInfo, OrderData>* provider, int sampleType)
{
...
float SignalCalculatorTest::get_fillImb(const QuoteInfo& quote, const TradeInfo& trade)
{
	float ret = 0.;
...
float SignalCalculatorTest::get_fillImb(TickProvider<TradeInfo,QuoteInfo,OrderData>* provider)
{
	QuoteInfo quote = provider->Bbo( (unsigned char)lastBookTrade_.flags );
	string market = string(1, lastBookTrade_.flags);
	if( valid_quote(quote) && quote.ask > quote.bid )
...
void SignalCalculatorTest::calculate_targets(hff::SigC& sig, TCM_classic& tcmc)
{
	const hff::NonLinearModel& nonLinMod = MEnv::Instance()->nonLinearModel;
	const hff::LinearModel& linMod = MEnv::Instance()->linearModel;
	...
	for( int k = 0; k < Npts; ++k )
	{
		...
		if( sI.gptOK == 1 )
		{
			int msecsFuture = sI.msso + linMod.openMsecs + 4 * 1000;
			QuoteInfo qFuture = tcmc.SingleConverter()->NbboAt(msecsFuture);
			sI.persistent = get_persistent(qFuture, sI);
			...
			for( int iT = 0; iT < nT; ++iT )
			{
				...
				{
					QuoteInfo quoteFrom = tcmc.SingleConverter()->NbboAt(msecsFrom);
					if( valid_quote(quoteFrom) )
						midFrom = get_mid(quoteFrom);
				}
				...
				QuoteInfo quoteTo = tcmc.SingleConverter()->NbboAt(msecsTo);
				if( valid_quote(quoteTo) )
					midTo = get_mid(quoteTo);
			...
			// to close.
			{
				...
				if( midFrom > 0. && midTo > 0. )
				{
					...
					if( horizon >= 60 && horizon < 600 )
						clip(sI.targetClose, om_target_clip_);
					else if( horizon >= 600 && horizon < 3600 )
						clip(sI.targetClose, tm_target_clip_);
					else if( horizon >= 3600 )
						clip(sI.targetClose, tm_target_60_clip_);
...
void SignalCalculatorTest::calculate_targets(hff::SigC& sig, const vector<QuoteInfo>& quotes)
{
	const hff::LinearModel& linMod = MEnv::Instance()->linearModel;
	const hff::NonLinearModel& nonLinMod = MEnv::Instance()->nonLinearModel;
	...
	for( int k = 0; k < Npts; ++k )
	{
		...
		if( sI.gptOK == 1 )
		{
			int secPersist = min(sec + 4, linMod.n1sec - 1);
			QuoteInfo qFuture = quotes[vQindx1s[secPersist]];
			sI.persistent = get_persistent(qFuture, sI);
...
void SignalCalculatorTest::calculate_tob_signals(hff::StateInfo& sI,
...
void SignalCalculatorTest::calculate_book_signals(hff::SigC& sig, hff::StateInfo& sI,
		TickProvider<TradeInfo,QuoteInfo,OrderData>* provider)
{
	if( sI.tobOK == 1 )
	{
...
void SignalCalculatorTest::calculate_mi_signals(hff::SigC& sig, hff::StateInfo& sI,
		TickProvider<TradeInfo,QuoteInfo,OrderData>* provider)
{
...
void SignalCalculatorTest::calculate_signals(hff::SigC& sig, hff::StateInfo& sI, int msecs)
{
	const hff::LinearModel& linMod = MEnv::Instance()->linearModel;
	float firstMidQt = get_mid(firstQuote_);
...
void SignalCalculatorTest::get_top_comp_book(QuoteInfo& compBkTop, TickProvider<TradeInfo,QuoteInfo,OrderData>* provider)
{
...
void SignalCalculatorTest::get_dsOL(TickProvider<TradeInfo,QuoteInfo,OrderData>* provider, QuoteInfo& quoteFrom, double& SaOL, double& SbOL)
{
