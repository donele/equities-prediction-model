#include <MOrders/MTradeEventAna.h>
...
// Purpose: analysis of self trading, especially CA trade event.
// Status: preliminary

MTradeEventAna::MTradeEventAna(const string& moduleName, const multimap<string, string>& conf)
...
MTradeEventAna::~MTradeEventAna()
{}

void MTradeEventAna::beginJob()
{
...
void MTradeEventAna::beginMarket()
{
...
void MTradeEventAna::beginMarketDay(string market, int idate)
{
	/*
	// TickAccess
	taQ_.Clear();
...
*/
	...
	get_close(mClose_, market, idate);
}

void MTradeEventAna::beginTicker(const string& ticker)
{
...
void MTradeEventAna::endTicker(const string& ticker)
{
...
void MTradeEventAna::endDay()
{
}

void MTradeEventAna::endMarket()
{
}

void MTradeEventAna::endJob()
{
}

void MTradeEventAna::set_ticker_list(int idate, string market)
{
	...
	*/
	...
	for( vector<vector<string> >::iterator it = vvs.begin(); it != vvs.end(); ++it )
	{
		...
		if( !symbol.empty() )
		{
			...
			//if( binary_search(selectedTickers.begin(), selectedTickers.end(), symbol) )
			{
...
/*
void MTradeEventAna::get_retON(string market, int idate)
{
...
void MTradeEventAna::read_orders(vector<vector<string> >& vvo, int idate, string market, string ticker)
{
	vvo.clear();
	string sExch = mto::code(market);
...
string MTradeEventAna::get_selMarket(string market)
{
	string sExch = mto::code(market);
...
void MTradeEventAna::select_orders(vector<vector<string> >& vvo, vector<MercatorOrder>& vMOrderBuy, vector<MercatorOrder>& vMOrderSell,
...
/*
void MTradeEventAna::set_latency_offset(int idate, string market)
{
	latency_offset_ = 0;
...
void MTradeEventAna::get_pnl(vector<MercatorOrder>& vMOrder, const string& market, const string& ticker, const string& bs)
{
	int mult = (bs == "buy") ? 1 : -1;
	int closeMsecs = mto::msecClose(market);
	double closePrice = mClose_[ticker];
	for( vector<MercatorOrder>::iterator itb = vMOrder.begin(); itb != vMOrder.end(); ++itb )
	{
		int orderMsecs = itb->orderMsecs;
		double retTarget = mult * (closePrice / itb->price - 1.) * basis_pts_;
		itb->gain = retTarget;
	}
}

/*
void MTradeEventAna::read_quote_info( vector<MercatorOrder>& vMOrder, string market, string ticker, string bs,
...
void MTradeEventAna::calc_returns(TickSeries<QuoteInfo>& tsQ, int orderMsecs, vector<MercatorOrder>::iterator& itb, const int mult, int closeMsecs, double reton)
{
	// Returns.
...
void MTradeEventAna::find_match(QuoteInfo& theQuote, TickSeries<QuoteInfo>& tsQ, int orderMsecs, vector<MercatorOrder>::iterator& itb, string bs)
{
...
bool MTradeEventAna::exchangeMatch(int tickEx, int dbEx)
{
	return true;
...
bool MTradeEventAna::is_valid( QuoteInfo& quote )
{
...
int MTradeEventAna::get_price_change( double last, double now )
{
	int ret = 0;
...
bool MTradeEventAna::valid_price( double price )
{
	bool valid = price > ltmb_ && price < 10000000;
	return valid;
}

void MTradeEventAna::get_o2x( vector<MercatorOrder>& vMOrder, int m1, int m2, double price, int priceDeteriorated )
{
