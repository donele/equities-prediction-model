#include <MOrders/MSelfDealingAna.h>
...
MSelfDealingAna::MSelfDealingAna(const string& moduleName, const multimap<string, string>& conf)
	:MModuleBase(moduleName, true),
	debug_(false),
	verbose_(0),
	minTimeSinceLastOrder_(10),
	execType_({'L'}),
	schedType_({1, 2, 4}),
	sign_({-1, 1}),
	fee_bpt_(0.),
...
MSelfDealingAna::~MSelfDealingAna()
{}

void MSelfDealingAna::beginJob()
{
	cout << moduleName_ << "::beginJob()" << endl;
	return;
}

void MSelfDealingAna::beginMarket()
{
...
void MSelfDealingAna::beginMarketDay()
{
	if(!condVar_.empty())
		tqi_ = static_cast<const TradeQuantileInfo*>(MEvent::Instance()->get("", "quantileInfo"));
}

void MSelfDealingAna::beginTicker(const string& ticker)
{
...
void MSelfDealingAna::endTicker(const string& ticker)
{
	return;
}

void MSelfDealingAna::endDay()
{
	string market = MEnv::Instance()->market;
	int idate = MEnv::Instance()->idate;
...
void MSelfDealingAna::endMarket()
{
...
void MSelfDealingAna::endJob()
{
	return;
}

void MSelfDealingAna::readClosePricePosition(map<string, CloseInfo>& mClose, int idate)
{
	mClose.clear();
	string market = MEnv::Instance()->market;
...
void MSelfDealingAna::intra_day_stat(const vector<MercatorTrade>* pvmt, const string& ticker)
{
	int idate = MEnv::Instance()->idate;
	string market = MEnv::Instance()->market;
	PnlSum psum;
	...
	for(int ti = 0; ti < tradeIndex.size(); ++ti)
	{
		...
		if(mt.msecs < lastMsecs + minTimeSinceLastOrder_ && mt.sign * lastSign < 0.)
		{
			if(mt.qty > 0)
			{
...
vector<int> MSelfDealingAna::selectTradeIndex(const vector<MercatorTrade>* pvmt)
{
...
void MSelfDealingAna::update_summary(PnlSummary& summary, PnlSum& sum)
{
