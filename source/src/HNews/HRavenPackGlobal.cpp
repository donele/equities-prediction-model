...
HRavenPackGlobal::HRavenPackGlobal(const string& moduleName, const multimap<string, string>& conf)
:HModule(moduleName),
verbose_(0),
...
HRavenPackGlobal::~HRavenPackGlobal()
{}

void HRavenPackGlobal::beginJob()
{
...
void HRavenPackGlobal::beginDay()
{
	...
	if( !ifs_.is_open() )
	{
		...
		sprintf(filename, "%s\\%d\\%d-%02d-macro.csv", input_dir_.c_str(), yyyy, yyyy, mm);
		ifs_.open(filename);
	}
...
void HRavenPackGlobal::beginMarket()
{
	int idate = HEnv::Instance()->idate();
			...
			if( msecs_valid(market, msecs, sl) )
			{
				...
				//if( binary_search(tickers.begin(), tickers.end(), ticker) )
				{
					if( preOpen ) // pre open.
						...
						cerr << "HRavenPackGlobal::beginMarket() ERROR.\n";
				}
				//else
...
void HRavenPackGlobal::endMarket()
{
	return;
}

void HRavenPackGlobal::endDay()
{
	return;
}

void HRavenPackGlobal::endJob()
{
	return;
}

bool HRavenPackGlobal::msecs_valid(string market, int msecs, vector<string>& sl)
{
	bool valid = true;
...
int HRavenPackGlobal::get_idate(string& line)
{
	int idate = 0;
	if( line.size() > 10 )
...
//vector<string> HRavenPackGlobal::get_tickers(string market, int idate)
//{
...
//string HRavenPackGlobal::get_ticker(string sl1, string market, int idate)
//{
...
QuoteTime HRavenPackGlobal::get_lineTime(string& t)
{
...
void HRavenPackGlobal::write_tickdata(map<string, TickSeries<QuoteInfo> >& mTs, string market, QuoteTime nextCloseUTC, int msecsFirstData)
{
...
int HRavenPackGlobal::get_msecs(QuoteTime qt, string market)
{
...
void HRavenPackGlobal::add_data(map<string, TickSeries<QuoteInfo> >& mTs, string ticker, int msecs, vector<string>& sl)
{
	if( true ) // v 3.0
	{
		int rel = atoi(sl[5].c_str()); // relevance.
		int cat = newsCatRPG_.cat2i(sl[11]); // Category.
