#include <HNews/HRavenPackAggMarketRet.h>
...
HRavenPackAggMarketRet::HRavenPackAggMarketRet(const string& moduleName, const multimap<string, string>& conf)
:HModule(moduleName),
verbose_(0),
interval_(1),
market_("")
...
HRavenPackAggMarketRet::~HRavenPackAggMarketRet()
{}

void HRavenPackAggMarketRet::beginJob()
{
...
void HRavenPackAggMarketRet::beginMarket()
{
	return;
}

void HRavenPackAggMarketRet::beginDay()
{
	int idate = HEnv::Instance()->idate();
	if( verbose_ > 1 )
		cout << market_ << "\t" << idate << endl;

	ret_ = OneSecRet(market_, idate);
	...
	//else if( "US" == market_ )
	//	outdir_ = "\\\\smrc-ltc-mrct40\\l\\tickC\\us\\news";
...
void HRavenPackAggMarketRet::endDay()
{
	return;
}

void HRavenPackAggMarketRet::endMarket()
{
	return;
}

void HRavenPackAggMarketRet::endJob()
{
	return;
}

string HRavenPackAggMarketRet::get_outdir()
{
	return ".\\US";
}

void HRavenPackAggMarketRet::process_market(int idate)
{
...
void HRavenPackAggMarketRet::reset_times(QuoteTime lineTimeUTC, QuoteTime& nextOpenUTC, QuoteTime& nextCloseUTC, int& msecsFirstData)
{
...
void HRavenPackAggMarketRet::aggregate(int idate)
{
	for( map<int, NewsAggregate>::iterator it = mNdayNewsagg_.begin(); it != mNdayNewsagg_.end(); ++it )
...
bool HRavenPackAggMarketRet::msecs_valid(int msecs, vector<string>& sl)
{
	bool valid = true;
...
int HRavenPackAggMarketRet::get_idate(string& line)
{
	int idate = 0;
	if( line.size() > 10 )
...
vector<string> HRavenPackAggMarketRet::get_tickers(int idate)
{
...
QuoteTime HRavenPackAggMarketRet::get_lineTime(string& t)
{
...
void HRavenPackAggMarketRet::write_tickdata(QuoteTime nextCloseUTC, int msecsFirstData, set<string>& sectors)
{
...
//void HRavenPackAggMarketRet::import_sectors(TickStorage<QuoteInfo>& tsQ, set<string>& sectors)
//{
...
void HRavenPackAggMarketRet::import_market(TickStorage<QuoteInfo>& tsQ, int idate)
{
	...
	ret_.calculate_each_return();

	// Calculate average returns.
	TickStorage<ReturnData> tsr;

	//if( averageReturn_ )
	{
		string avgRetName = market_ + "_FUT";
		ret_.calculate_average_return(); // All the tickers inuniverse.
		ret_.insert_average_return(tsr, avgRetName);
	}
	//else
	//	ret_.insert_each_return(tsr);

	// Write the file.
	char buf[200];
	sprintf( buf, "%d Writing in the directory %s\n", idate, outdir_.c_str() );
	cout << buf;
	tsr.WriteBIN(idate, outdir_.c_str(), mto::longTicker(market_));
...
int HRavenPackAggMarketRet::get_msecs(QuoteTime qt)
{
...
void HRavenPackAggMarketRet::add_data(string ticker, string sector, int msecs, vector<string>&sl)
{
	int rel = atoi(sl[3].c_str());
...
//void HRavenPackAggMarketRet::add_tickers_without_news(int idate)
//{
...
HRavenPackAggMarketRet::NewsAggregate::NewsAggregate(string market, int nDayAgg, int interval)
:market_(market),
nDayAgg_(nDayAgg),
interval_(interval)
{}

void HRavenPackAggMarketRet::NewsAggregate::beginDay(int idate, int msecOpen, int msecClose, int nInterval)
{
	msecOpen_ = msecOpen;
...
void HRavenPackAggMarketRet::NewsAggregate::add_data(string ticker, string sector, int msecs, vector<string>& sl)
{
	int rel = atoi(sl[3].c_str());
	int ess = atoi(sl[5].c_str());
	int ens = atoi(sl[6].c_str());
...
int HRavenPackAggMarketRet::NewsAggregate::get_volume(string ticker)
{
	Acc& accVol = tickerVol_[ticker];
...
int HRavenPackAggMarketRet::NewsAggregate::get_ticker_sent(string ticker)
{
	Acc& acc = tickerAcc_[ticker];
...
int HRavenPackAggMarketRet::NewsAggregate::get_ticker_sent_lag(string ticker)
{
	DailySummary& ds = tickerDailySummary_[ticker];
	int lag = ds.agg.mean();
	return lag;
...
void HRavenPackAggMarketRet::NewsAggregate::get_tickers_without_news(vector<string>& tickers)
{
...
double HRavenPackAggMarketRet::NewsAggregate::get_avg_sent(DailySummary& ds, Acc& acc)
{
	Acc sum = ds.agg + acc;
	double avg = sum.mean();
	return avg;
...
vector<int> HRavenPackAggMarketRet::NewsAggregate::get_sector_sent(string sector)
{
	vector<int> ret;
	map<string, vector<Acc> >::iterator it = mvSectorAcc_.find(sector);
	if( it != mvSectorAcc_.end() )
...
vector<double> HRavenPackAggMarketRet::NewsAggregate::get_market_sent()
{
	vector<double> ret;
...
void HRavenPackAggMarketRet::NewsAggregate::aggregate(int idate)
{
...
//void HRavenPackAggMarketRet::NewsAggregate::add_acc_today(DailySummary& ds, int idate, string ticker)
//{
//	Acc& acc = tickerAcc_[ticker];
...
void HRavenPackAggMarketRet::NewsAggregate::aggregate(DailySummary& ds, int outOfAggDate)
{
	Acc accAgg;
...
void HRavenPackAggMarketRet::NewsAggregate::add_acc_today_market(int idate)
{
	Acc sumDay;
	for( vector<Acc>::iterator it = vMarketAcc_.begin(); it != vMarketAcc_.end(); ++it )
	{
