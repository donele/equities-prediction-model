...
MPseudoTradeAdaptVol::MPseudoTradeAdaptVol(const string& moduleName, const multimap<string, string>& conf)
...
previous_position_(0.)
{
	...
	if( conf.count("nP") )
		nP_ = atoi(conf.find("nP")->second.c_str());
...
MPseudoTradeAdaptVol::~MPseudoTradeAdaptVol()
{}

void MPseudoTradeAdaptVol::beginJob()
{
	...
	for( int i = 0; i < nP_; ++i )
		thresInfo_.push_back(ThresInfo(thres_seed_, learning_rate_seed_));
...
void MPseudoTradeAdaptVol::beginDay()
{
	...
	for( int i = 0; i < nP_; ++i )
		thresInfo_[i].beginDay();
	//trades_.clear();

	charaInfo_.read(market_, idate, "medvolume", nP_, "uid");
}

void MPseudoTradeAdaptVol::market_close(DaySum& daysum, ofstream& ofsDebug, int idate)
{
	// Clcl.
...
void MPseudoTradeAdaptVol::thres_ana()
{
	for( int iBin = 0; iBin < nP_; ++iBin )
	{
		int N = 10;
		double pnl0 = 0.;
		OLS ols(1, 0, 2);

		vector<TradeMade> trades = thresInfo_[iBin].trades;

		for( double i = 0; i < N; ++i )
		{
			double extra_thres = i * thres_step_;
			double total_thres = extra_thres + thresInfo_[iBin].thres;
			double pnl = 0.;
			for( vector<TradeMade>::iterator it = trades.begin(); it != trades.end(); ++it )
			{
		...
		if( abs(coeffs[0] + 100.) > 1e-4 && abs(coeffs[1] + 100.) > 1e-4 )
		{
			double diff = thresInfo_[iBin].learning_rate * coeffs[1];
			...
			{
				diff = 0.5;
				thresInfo_[iBin].learning_rate *= 0.5;
			}
			else if( diff < -0.5 )
			{
				diff = -0.5;
			...
			{
				thresInfo_[iBin].learning_rate *= 1.2;
			}

			if( thresInfo_[iBin].thres + diff > max_thres_ )
			{
				diff = 0.;
				thresInfo_[iBin].learning_rate *= 0.5;
			}
			else if( thresInfo_[iBin].thres + diff < - max_thres_ )
			{
				diff = 0.;
				thresInfo_[iBin].learning_rate *= 0.5;
			}

			thresInfo_[iBin].thres += diff;
		}
	}

	printf("%s thres", moduleName_.c_str());
	for( int iBin = 0; iBin < nP_; ++iBin )
	{
		printf(" %.2f", thresInfo_[iBin].thres);
	}
	printf("\n");
}

void MPseudoTradeAdaptVol::endDay()
{
	const hff::LinearModel& linMod = MEnv::Instance()->linearModel;
	int idate = MEnv::Instance()->idate;
	...
	for( map<string, vector<MReadTickerState::TickerState> >::const_iterator itt = mTickerState->begin(); itt != mTickerStateEnd; ++itt )
	{
		...
		for( vector<MReadTickerState::TickerState>::const_iterator it = itt->second.begin(); it != ittSecondEnd; ++it )
		{
			...
			int bs = 0;
			int nshare = 0;
			if( ts.pred + restoreForce > ts.cost + thresInfo_[iBin].thres / basis_pts_ ) // buy.
			{
				price = ts.ask * exchrat; // forex
				bs = 1;
				nshare = min(maxShrChg_, ts.askSize);
			}
			else if( - (ts.pred + restoreForce) > ts.cost + thresInfo_[iBin].thres / basis_pts_ ) // sell.
			{
			...
			if( bs != 0 && mid > 0. && nshare != 0 )
			{
				...
				thresInfo_[iBin].trades.push_back( TradeMade(maxThres, pnl) );
...
void MPseudoTradeAdaptVol::endJob()
{
	char buf[1000];
