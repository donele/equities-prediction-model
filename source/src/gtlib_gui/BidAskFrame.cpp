...
nmarkets(0)
{
	...
	fEcanvasDist = new TRootEmbeddedCanvas("EcanvasDist",this,800,500);
	AddFrame(fEcanvasDist, new TGLayoutHints(kLHintsExpandX| kLHintsExpandY,10,10,10,1));

	// hframe1
	TGHorizontalFrame *hframe1 = new TGHorizontalFrame(this,800,80);
...
void BidAskFrame::InitVars() {
  ...
  if( homeDir != nullptr )
	  confPath = (string)homeDir + "/scripts/gui/bidask.conf";
  else
	  confPath = "bidask.conf";

  ifstream ifs(confPath.c_str());
  if( ifs.is_open() )
  {
	  TString market = "";
	  TString feed = "";
...
void BidAskFrame::set_market(int id)
{
  ...
  if( 1 )
  {
	  ...
	  int i = 0;
	  for( fs::directory_iterator itr(dir); itr != end_itr; ++itr, ++i )
	  {
		  if( fs::is_regular_file(itr->status()) )
		  {
			  string filename = itr->path().filename().string();
			  int pos = filename.find("20");
			  int idate = atoi(filename.substr(pos, 8).c_str());
			  TString date = itos(idate).c_str();
