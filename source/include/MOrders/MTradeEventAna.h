#ifndef __MTradeEventAna__
#define __MTradeEventAna__
...
class CLASS_DECLSPEC MTradeEventAna: public MModuleBase {
public:
	MTradeEventAna(const std::string& moduleName, const std::multimap<std::string, std::string>& conf);
	virtual ~MTradeEventAna();
...
private:
	...
	std::map<std::string, double> mClose_;
	...
	void get_pnl(std::vector<MercatorOrder>& vMOrder, const std::string& market, const std::string& ticker, const std::string& bs);
