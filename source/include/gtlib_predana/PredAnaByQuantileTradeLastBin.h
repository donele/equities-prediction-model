#ifndef __gtlib_sigread_PredAnaByQuantileTradeLastBin__
#define __gtlib_sigread_PredAnaByQuantileTradeLastBin__
...
class PredAnaByQuantileTradeLastBin {
public:
	struct LastBinTrade {
		LastBinTrade(int m, float p, int s):msecs(m),price(p),sign(s){}
		int msecs;
		float price;
		int sign;
	};
	PredAnaByQuantileTradeLastBin(){}
	PredAnaByQuantileTradeLastBin(const std::string& baseDir, const std::string& fitDir,
	...
	PredAnaByQuantileTradeLastBin(const std::string& baseDir, const std::string& fitDir, const std::string& model,
	...
	std::set<std::string> getTickers(int idate);
	std::vector<PredAnaByQuantileTradeLastBin::LastBinTrade> getTrades(int idate, const std::string& ticker);
...
private:
	...
	std::string statDir_;
	std::unordered_map<int, std::set<std::string>> mDateTickers_;
	std::unordered_map<int, std::unordered_map<std::string, std::vector<LastBinTrade>>> mmTrades_;

	std::vector<std::string> vTicker_;
	std::vector<int> vDate_;
	...
	void getAna(const std::vector<float>* vControl, int iB, int nBins, const std::vector<int>& vIndx);
