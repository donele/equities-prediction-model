#ifndef __gtlib_sigread_PredAnaByNTreeMultTarV2__
#define __gtlib_sigread_PredAnaByNTreeMultTarV2__
...
class PredAnaByNTreeMultTarV2 {
public:
	PredAnaByNTreeMultTarV2():debug_(false){}
	PredAnaByNTreeMultTarV2(const std::string& baseDir, const std::string& fitDir,
			...
			const std::string& fitDesc, float flatFee, bool debug = false);
	PredAnaByNTreeMultTarV2(const std::string& baseDir, const std::string& fitDir, const std::string& model,
			const std::string& targetName, int udate, const std::vector<int>& testdates,
			const std::string& fitDesc, float flatFee);
	~PredAnaByNTreeMultTarV2(){}
	void anaNTree();
	void anaThres(const std::vector<float>& vThres = std::vector<float>());
	void anaDay();
...
private:
	...
	void init(std::vector<PredWholeDay*>& vPred);
	...
	void getData(std::vector<gtlib::PredWholeDay*>& vPred);
	void fetchData(std::vector<PredWholeDay*> vPred, const std::string& ticker, int msecs);

	PObjMultTar getAnaNTree(int iTO, int iTT);
	std::string getOutputPath();

	std::vector<PObjMultTar> getAnaThres(int iTO, int iTT);
