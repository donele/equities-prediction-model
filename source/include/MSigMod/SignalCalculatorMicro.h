#ifndef __SignalCalculatorMicro__
#define __SignalCalculatorMicro__
...
class SignalCalculatorMicro: public TCS_book_micro {
public:
	int quoteSampleStep_;
	int tradeSampleStep_;
	int booktradeSampleStep_;

	SignalCalculatorMicro();
	SignalCalculatorMicro(int idate, const std::string& uid, const std::string& ticker, hff::SigC& sig,
			Sessions* sessions, int openMsecs, int closeMsecs, const std::string& auctionMkts,
			const std::vector<hff::OrderQty>* tradeQty);
	virtual ~SignalCalculatorMicro();
...
private:
	...
	//int msecsLastEvent_;
	int validt_;
	float max_trade_qty_rat_;
	float max_trade_price_rat_;
	float lastFillImb_;
	float high_;
	float low_;
	float high900_;
...
{
public:
	TaskPersistence(SignalCalculatorMicro* sigcal, int iSig);
	virtual TaskPersistence* MakeCopy(void);
	virtual void Execute(UsecsTime ti);
private:
	int iSig_;
	SignalCalculatorMicro* sigcal_;
};
