#ifndef __MTickReadSimple__
#define __MTickReadSimple__
...
class CLASS_DECLSPEC MTickReadSimple: public MModuleBase {
public:
	MTickReadSimple(const std::string& moduleName, const std::multimap<std::string, std::string>& conf);
	virtual ~MTickReadSimple();
...
private:
	...
	void set_directories(const std::string& market);
...
template<class T>
void MTickReadSimple::read_data(const std::vector<std::string>& dirs, const std::string& name, int& n, const std::string& ticker, int idate)
{
	// Read
