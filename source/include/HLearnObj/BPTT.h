...
namespace hnn {
	class CLASS_DECLSPEC BPTT : public NNBase {
		friend class BPTT;
	public:
		BPTT();
		BPTT(const BPTT & rhs);
		BPTT(std::vector<int>& N, double learn, double wgtDecay, double maxWgt, int verbose, double biasOut, double costMult,
			double sigP, double poslim, double maxPos, int lotSize, bool maxRet, bool noTwoTradesAtSamePrice);
		virtual ~BPTT();

		virtual BPTT* clone() const { return new BPTT(*this); }
