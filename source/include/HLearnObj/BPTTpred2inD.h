#ifndef __HNN_BPTTpred2inD__
#define __HNN_BPTTpred2inD__
#include "NNBase.h"
...
namespace hnn {
	class CLASS_DECLSPEC BPTTpred2inD : public NNBase {
		friend class BPTTpred2inD;
	public:
		BPTTpred2inD();
		BPTTpred2inD(const BPTTpred2inD & rhs);
		BPTTpred2inD(std::vector<int>& N, double learn, double wgtDecay, double maxWgt, int verbose, double biasOut, double costMult,
			double sigP, double poslim, double maxPos, int lotSize, bool maxRet, bool noTwoTradesAtSamePrice);
		virtual ~BPTTpred2inD();

		virtual BPTTpred2inD* clone() const { return new BPTTpred2inD(*this); }
